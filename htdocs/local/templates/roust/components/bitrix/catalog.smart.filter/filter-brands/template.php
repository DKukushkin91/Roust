		...
		<form name="<?echo $arResult["FILTER_NAME"]."_form"?>" action="<?echo $arResult["FORM_ACTION"]?>" method="get" class="smartfilter">
			...
			<div class="row">
				...
				foreach($arResult["ITEMS"] as $key=>$arItem)
				{
					...
					{
						if (empty($arval["DISABLED"])){$enable=true;}
					}
					?>				
					<?if (($i % $count_filter) == 0):?>
					<div class="filter__m-col">
           			<?endif;?>
						<div class="filter__group bx-filter-parameters-box" <?if ($enable!=true):?>style="display:none;"<?endif?>>
							<div class="filter__title bx-filter-parameters-box-title" onclick="smartFilter.hideFilterProps(this)">
									<?=$arItem["NAME"]?>
							...
							<div class="bx-filter-block">
								...
								switch ($arItem["DISPLAY_TYPE"])
								{
									...
									default://CHECKBOXES
										?>
											<?foreach($arItem["VALUES"] as $val => $ar):?>
											<?if ($ar["VALUE"]=="Для щенков/котят"){											
												$url = $_SERVER['REQUEST_URI'];
												$findme = '/dogs/';$pos = strpos($url, $findme); //щенки и котята
												if($pos){$ar["VALUE"] = "Для щенков";}
												else {$ar["VALUE"] = "Для котят";}
											}?>
												<div class="filter__input" <? echo $ar["DISABLED"] ? 'style="display:none;"': '' ?>>
													<div class="checkbox">
													<input 
																	type="checkbox" 
																	...
																	onclick="smartFilter.click(this)"
																/>
														<label data-role="label_<?=$ar["CONTROL_ID"]?>" class="bx-filter-param-label checkbox__label ga-click-js-filter <? echo $ar["DISABLED"] ? 'disabled': '' ?>" data-ga-category="catalog-<?=$url_category?>" data-ga-action="filter" data-ga-label="<?=$arItem["NAME"]?> - <?=$ar["VALUE"];?>" for="<? echo $ar["CONTROL_ID"] ?>">
															<span class="bx-filter-input-checkbox">															
					...
					<?if (($i % $count_filter) == $count_filter-1):?>
					</div>
					<?endif;?>
				<?
				$i++;
